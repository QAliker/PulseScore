...
describe('H2hService', () => {
  ...
  beforeEach(() => {
    ...
    mockNormalizer = { normalizeMatch: jest.fn((m: any) => ({ externalId: m.match_id })) };
  ...
  it('should fetch H2H from API when cache is empty', async () => {
    mockClient.get.mockResolvedValue({ firstTeam_VS_secondTeam: [{ match_id: '1' }], firstTeam_lastResults: [{ match_id: '2' }], secondTeam_lastResults: [{ match_id: '3' }] });
    const result = await service.getH2H('2627', '2637');
    expect(mockClient.get).toHaveBeenCalledWith('get_H2H', { firstTeamId: '2627', secondTeamId: '2637' });
